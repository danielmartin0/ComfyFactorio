...
local output = {
	...
	command_needs_validation = "This command requires validation to run. Please re-run the command if you wish to proceed.",
	command_needs_custom_validation = "This command requires validation to run. %s - please re-run the command if you wish to proceed.",
...
local function execute(event)
	...
	if validate_self and not command_data.validated_command then
		...
		if command_data.custom_message then
			handle_error(
				string.format(output.command_needs_custom_validation, command_data.custom_message),
	...
	local processed_input = input_text:gsub('"([^"]-)"', function(segment)
...
function Public.new(name, help)
	...
	if game then
		error("Cannot run new() when game is initialized : " .. name, 2)
	end
...
Public.new("get", "Hover over an object to get its name.")
	...
	:callback(function(player, action)
		...
		player.print(
			"[color=orange]Direction:[/color] " .. entity.direction .. " (" .. directions[entity.direction] .. ")"
		)
